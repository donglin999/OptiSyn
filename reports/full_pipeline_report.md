...
## 第 0.5 章 — 假设驱动 + 逐步逼近的逻辑链路 ⚙

> 本章在数据展开前，先把"为什么走这条路、每步靠什么把假设往前推一节、哪些地方需要谨慎"全部摊开。供你在阅读后续章节时随时回查。

### 0.5.1 你的初始假设（项目立项时锁定）

**核心命题**: 流感感染 → 外周迷走神经感觉端激活 → 中枢 NTS Tac1+Vglut2+ 神经元 → NTS Cx3cr1+ 微胶质细胞 → 神经-免疫互作介导咳嗽病理

**三段拆解**:

| 段 | 物理位置 | 待证 |
|---|---|---|
| ① 外周端 | 肺组织 (Mo / Mφ / 上皮) + 迷走神经节 | 流感期间存在"保护轴失效"的可量化分子签名 |
| ② 神经传输端 | 迷走 → NTS 投射 | 外周信号有合理的配-受体载具进入中枢 |
| ③ 中枢端 | NTS 神经元-微胶质 | 中枢有"接收 + 反馈"的双向通讯回路 |

### 0.5.2 数据集 ↔ 假设段对应

| 段 | 待证 | 数据集 | 章节 | 通过判据 |
|---|---|---|---|---|
| ① | "Mo 端有保护轴崩塌" | GSE42639 (5 cells × 5 dose) | 第 2 章 | 5 步 + 严选稳定收敛到 ≤100 候选 |
| ① | "崩塌跨毒株可重复" | GSE31022 (D1-D6 时序) | 第 3 章 | 双标基因 ≥ 1 |
| ② | "迷走端有受体接收 / 配体投射" | GSE161878 (vagus bulk) | 第 4 章 | 关键 LR 配/受体在迷走样本可检 |
| ②③ | "vagus → Mo / NTS 的 LR 严判" | CellChatDB | 第 6 章 | 至少 1 条高可信 LR 通路 |
| ③ | "NTS Tac1+ ↔ Cx3cr1+ 双向通讯 + Tac1+ 细胞特异" | GSE268741 (NTS sc) | 第 7 章 | 5 模块均通过 |
| ① | "外周 Mo Cx3cr1 单细胞验证 + RTX 救援" | GSE296065 (lung sc) | 第 5 章 | 至少 1 个候选通过 BH 双判 |

### 0.5.3 逐步逼近 — 每一步的"上一步推出 → 这一步问什么"

```
[Step 1] GSE42639 5 cells × 5 dose 全表 → 选 Mo
   ↓ 上一步逻辑: 假设 ① 要求"外周髓系存在保护轴失效", 必须先确认是哪种细胞最锐利
   ↓ 这一步问: 5 cells 哪个的"轻症维持→重症崩塌"模式最一致 + 候选数最稳定?
   答: Mo (DEG 数 9144, 严选 65, 候选生物学富集免疫调控) — 详 §2.0 + §9.4-X

[Step 2] Mo 5 步流程 → 564 候选, 严选叠加 → 58
   ↓ 上一步逻辑: Mo 是最强细胞, 现在要把"保护性靶点"具体到基因
   ↓ 这一步问: 哪些基因满足"轻症维持 (TX91) → 剂量响应崩塌 (PR8_0p6→100)"且不是被剔除的代谢酶?
   答: 564 → 65 → 58 (含 Cx3cr1 / Klf2 / Arrb1 / Btla / Cd209a / Il16 等保护轴枢纽)

[Step 3] GSE31022 D1-D6 跨毒株时序 → 4 双标
   ↓ 上一步逻辑: 58 个是 GSE42639 单数据集结果, 必须在另一份毒株 + 时间序列里复现
   ↓ 这一步问: 58 个里哪些在 D1-D6 全周期持续下调 + 任一日相对 Ctrl 显著下调?
   答: Gstm1 / Dusp18 / Insr / 6430548M08Rik (强稳健核)
   ⚠️ 4/55 是较小的子集 — 大多数 58 候选是"GSE42639 剂量响应特异", 不一定时序复现; 不是失败, 是 specificity 不同

[Step 4] GSE161878 vagus DESeq2 → 144 上 / 10 下
   ↓ 上一步逻辑: 假设 ② 要求迷走端能接收外周信号, 先看迷走自身在感染下表达谱
   ↓ 这一步问: 迷走神经节里哪些基因变化? 是否含 Mo 候选的对应配/受体?
   答: 上调以髓系浸润 + MHC-II + IFN 抗病毒为主, **vagal nociceptor 标志 Gpr151 上调** (FDR=1.7e-4)
   ⚠️ Mo 端 Cx3cr1 / Btla / Il16 受体在 vagus 不显著上调 — 这是 v1 CellChat 失败的根因

[Step 5 — 决定性转折] CellChat v1 失败 → v2 放宽失败 → v3 反向 ★
   ↓ 上一步逻辑: 假设原始方向是 vagus → Mo. 但 Tnfrsf14 / Cd4 受体在 vagus 21/21 样本 CPM<1
   ↓ 这一步问: 是否原始方向反了? 配体 Cx3cl1 在 vagus 是否固有表达?
   答: vagus Cx3cl1 CPM 2.5–8.5 21/21 样本固有表达 + Mo Cx3cr1 重症崩塌 (slope=-0.80)
   ⚠️⚠️ 这是关键的"假设细化": 原始方向"vagus → Mo"未通过严判, 反向"vagus 释放 Cx3cl1 → 外周 Mo + 中枢 Cx3cr1+ 微胶质" 走通
       — 不是假设错, 是配-受体的"配体端在 vagus 而不是受体端", 但需要承认这是事后修正 (post-hoc refinement), 不是 a priori 设计

[Step 6] GSE268741 NTS sc — 5 模块依次推进
   ↓ 上一步逻辑: 既然 vagus 能投射 Cx3cl1, 中枢 NTS 端必须有匹配的接收 + 反馈
   ↓ 模块 1: NTS 是否能分出 Tac1+Vglut2+ 与 Cx3cr1+ 两群?  → 是 (212 + 215 cells)
   ↓ 模块 4: 两群是否存在 LR 互作?                           → Cx3cl1→Cx3cr1 prob=0.602 ★
   ↓ 模块 5: Tac1+ 是否在 LR + 咳嗽通路双轴显著领先?         → 是 (双轴第一)
   ⚠️ 单 GSM 样本 (1 个 GSE268741), 严格意义上属 pseudoreplication

[Step 7] GSE296065 lung sc + RTX 救援 → 1 高可信 (Cx3cr1)
   ↓ 上一步逻辑: 完成中枢闭环后, 回到外周做单细胞 + 药理验证
   ↓ 这一步问: 58 候选在单细胞 Mo 上能特异性表达 + RTX 阻断后回升的有几个?
   答: 28 通过 specificity, 仅 Cx3cr1 通过 BH 双判 (V7 vs R7 log2FC=0.86)
   ⚠️ 28 → 1 的衰减提示 RTX 救援不是普遍现象, 而是聚焦在 Cx3cr1 这一枢纽节点上
```

### 0.5.4 全流程异常预警 — 9 项需要你及时处理

> 以下是审稿人/同行评议**最可能挑战**的点，按严重度排序。每条都给了**当前状态**和**建议动作**。

| # | 风险 | 严重度 | 当前状态 | 建议动作 |
|---|---|---|---|---|
| 1 | **GSE268741 单 GSM 无生物学重复** | 🔴 严重 | 9638 cells 全来自 1 GSM, 所有细胞间比较是 pseudoreplication | 论文必须明确标注; **务必寻找第 2 份独立 NTS sc 数据**（如 Allen Brain / GSE 其它 NTS）做补充验证 |
| 2 | **CellChat v1 阴性 → v3 反向是事后修正** | 🟠 中等 | 严选 v1 阴性后切换 v3 方向, 是 post-hoc rescue | 论文里必须如实描述顺序; 最好补充独立的 vagus-Cx3cl1 蛋白染色或 in-situ 证据 |
| 3 | **GSE161878 D-suffix 推断** | 🟡 低 | 从样本名后缀推断 AM312D=AM312 同动物重复, 原 metadata 未明示 | 联系 GEO 投稿者邮件确认; 若不可达, 论文里同时给出 11+13 含 D 与 11+10 剔 D 双版本 |
| 4 | **RTX 救援 28→1 衰减** | 🟠 中等 | 58 候选只有 Cx3cr1 通过 BH 双判 | **不要**把 58 严选都包装成 RTX 靶点; 论文必须明确表述"Cx3cr1 是核心枢纽, 其余 57 个 RTX 响应弱或未达显著" |
| 5 | **TX91 是带毒 baseline 而非纯阴性** | 🟡 低 | "轻症维持→重症崩塌"的 baseline 是 H1N1 弱毒, 不是 Sham | Figure legend 必须写清"剂量响应基线为 TX91"; 不能跟 Sham vs 感染混用 |
| 6 | **Cx3cr1+ microglia pct=100% 是定义同义反复** | 🟡 低 | 215 cells 按 Cx3cr1 expr>0 定义后, pct=100% 是 tautological | 论文表述"215 cells 是 Cx3cr1+ 定义池", 不能写"100% 表达验证" |
| 7 | **Mo Cx3cr1 slope=-0.80 参考点是 TX91** | 🟡 低 | 用 TX91 当 t=0 算 slope; Sham mean=10.05 / TX91=9.88 / PR8_100=7.50 | 数字本身正确, 但报告 slope 时同时给 Sham 基线参照 |
| 8 | **5 cells StepA 严选两两交集 = 0** | 🟠 中等 | 解读为"细胞各看不同机制"; 但也可能是 5 步流程对细胞特异性过敏 | 论文里给两种解读并行 + 提供 sensitivity 分析 (如对单 cell 类内排序而非阈值) |
| 9 | **同事用 detection.tsv 跑出 Ly 是噪音** | ✅ 已诊断 | 见 §9.4-Y / §9.4.Y.2; 同事方法 5×4 全表共 5 DEG | 已写诊断报告; 把修复建议给到同事即可 |
...
### 2.0 为什么是 Mo？— 5 cells 对称比较的决策依据

> 这一节回答"在 GSE42639 的 5 种细胞 (Nh / Am / Ne / Mo / Ly) 中为什么主分析锁定 Mo"。完整对称化实验在 §9.4-X / §9.4-Y, 此处给决策摘要。

#### 2.0.1 候选细胞清单（按数据集元数据）

| 简码 | 文献定义 | 总样本 (5 组×3 重复) |
|---|---|---|
| **Nh** | CD45neg 上皮 / 内皮 / 基质 | 15 |
| **Am** | 肺泡巨噬细胞 (alveolar Mφ) | 15 |
| **Ne** | 中性粒 | 15 |
| **Mo** | Ly6Chi 单核髓系 (单核 + Mo-derived Mφ) | 15 |
| **Ly** | B + T + NK 混合淋巴 | 15 |

#### 2.0.2 三层决策证据（按强度）

**Layer 1 — 统计锐度 (5 cells × 4 contrasts 全表对称)**

| Cell | TX91 vs Sham | PR8_0.6 vs Sham | PR8_10 vs Sham | PR8_100 vs Sham | **总和** |
|---|---:|---:|---:|---:|---:|
| **Mo**  | 621 | 2503 | 2814 | **3206** | **9144** ★ |
| Ne | 232 | 1021 | 1956 | 2206 | 5415 |
| Nh | 152 | 773 | 1891 | 1555 | 4371 |
| Am | 134 | 689 | 1480 | 1596 | 3899 |
| Ly | 105 | 641 | 542 | 739 | 2027 |

> Mo 在 4 个 contrasts **全部第一**, 领先第二名 1.6×–2.7×。不是边缘领先。

**Layer 2 — 严选稳健性 (5 cells 各跑同一份 5 步 + StepA 严选)**

| Cell | 5 步 → 564 ✓ Mo | StepA 严选 | 关键基因 |
|---|---:|---:|---|
| **Mo** | 564 | **65** ★ | **Cx3cr1 / Klf2 / Arrb1 / Btla / Cd209a / Il16** (经典保护轴) |
| Am | 588 | 31 | Trem2 / Ramp1 / Anxa3 / Gstm1 / 8 H2 组蛋白 (alveolar Mφ 程序) |
| Nh | 410 | 13 | 上皮屏障类 |
| Ly | 40 | 3 | 体量太小, 不构成稳健模块 |
| Ne | 214 | 2 | 严选后几乎归零 |

**Layer 3 — 假设契合度 (回到第 0.5 章核心命题)**

- 项目假设的"中央信号轴"是 **Cx3cl1-Cx3cr1**
- **Mo 是 Cx3cr1 受体端**, 重症剂量响应 slope = **−0.80** (5 cells 中最深)
- **Am 是 Cx3cl1 配体端**, slope = **−0.60** (5 cells 中第二)
- 因此 Mo 是"**信号接收端崩塌**"的最佳代表细胞 → 锁定为主分析

#### 2.0.3 为什么不是其它 4 个 cell？

| Cell | 不选的原因（生物学 + 统计） |
|---|---|
| Am | 是平行有效细胞（Cx3cl1 配体端崩塌），不是"不对"，是**配套补充**。论文中**建议作为 Cx3cl1 端**与 Mo（Cx3cr1 端）并列展示，构成"双端瓦解"图谱 |
| Nh | CD45neg 混合体（上皮 + 内皮 + 基质），生物学异质度大，候选解释困难 |
| Ne | 中性粒在感染晚期数量爆增 → "DEG 数高"很多是**细胞数偏倚**而非真实细胞内程序变化；严选后只剩 2，无稳健模块 |
| Ly | 总 DEG 数 5 cells 倒数第一 (2027)，StepA 仅 3 个，体量不构成保护轴；同事用错误方法看到的"Ly 主导"是 TX91 contrast 下的 1 个噪音 DEG（详见 §9.4.Y.2）|

#### 2.0.4 决策结论

**Mo 是首选主分析细胞**（统计 + 生物学 + 假设契合三层一致）。
**Am 是配套平行视角**，不是冲突，建议论文里并列展示。
**Nh / Ne / Ly 不构成独立保护轴**，作为 5 cells 对称对照即可。

> **诚实标注**: "5 cells StepA 候选两两交集 = 0" 这个发现可解读为"不同细胞看不同机制"（我们的解读），也可解读为"5 步流程的阈值组合对细胞特异性过敏"。论文中宜两种解读并陈，并补 sensitivity 分析（如改用 within-cell ranking 而非全局阈值）。详见 §0.5.4 风险 #8。
...
### 9.6 思路链路客观性评审 + 异常预警逐项详表

> 本节对每一个**可能引起审稿人/同事/同行质疑**的点，提供：现象 → 是否真异常 → 严重度 → 建议处理。
> 与 §0.5.4 的精简表呼应，此处展开机制 + 处理路径。

---

#### Issue #1 — GSE268741 单 GSM 限制因果推断 🔴 严重

**现象**: NTS 单细胞数据 9638 cells **全部来自 1 个 GSM**（Gannot 2024 公开数据当前可获 1 个）。
**机制**: 所有"细胞类型间统计比较"（Tac1+ vs Cx3cr1+ vs Other）在严格意义上是 **pseudoreplication** — 同一动物里采的不同细胞算 n。
**是否真异常**: **真**。这是统计推断层面的结构性局限，不是脚本错误。
**当前缓解**: 在模块 5 排他性测试用 **wilcox + 4 维独立指标** 增强证据；在模块 4 的 LR 强度按 Hill 函数计算细胞-细胞概率（不是样本-样本统计）。
**建议处理**:
1. **论文里 figure legend / 方法学限制段必写**: "single donor, biological replication pending"
2. **主动寻找第 2 份独立 NTS sc 数据**: Allen Brain Cell Atlas / Mouse Brain Atlas / GSE 系列里再找 1 份 NTS 区域单细胞
3. 实验室如能拿到独立 NTS sc 实验机会, 优先级最高

---

#### Issue #2 — CellChat 假设方向调整是事后修正 🟠 中等

**现象**: 原始假设是"vagus 释放配体 → 外周 Mo / NTS 受体接收"。CellChat v1 (Mo→Vagus 严判) 0 命中，v2 (放宽阈值) 1 命中，v3 切到反向 Cx3cl1→Cx3cr1 才走通 (prob=0.602)。
**机制**: v1 阴性的根因——**Tnfrsf14 / Cd4 等受体在 vagus 21/21 样本 CPM<1 真不表达**（不是数据问题，是迷走神经节本来就不富集这类受体）。反向后发现 Cx3cl1 配体反而是 vagus 固有表达的（CPM 2.5–8.5 21/21 样本），于是把"vagus 是配体源"作为新假设。
**是否真异常**: **不算严重错误**，但属于 **post-hoc refinement**。从科学严谨标准看，需在论文中如实描述顺序 — 否则会被读者误以为反向 LR 是 a priori 设计。
**当前缓解**: 反向 LR 在 GSE268741 NTS sc 模块 4 也独立出现 (Cx3cl1→Cx3cr1 prob=0.602)，是双数据集独立支持的，弥补了 post-hoc 风险。
**建议处理**:
1. 论文 Methods 必须按时间顺序写: "We first tested Mo→Vagus direction (v1, null result); then refined hypothesis to Cx3cl1→Cx3cr1 reverse direction based on the empirical observation that vagal Cx3cl1 is constitutively expressed."
2. 若有条件，补 1 份 vagus Cx3cl1 蛋白染色 (IHC) 或 in-situ hybridization 作独立验证

---

#### Issue #3 — GSE161878 D-suffix 是推断不是注释 🟡 低

**现象**: AM312D / AM313D / AM314D 在原 metadata 里没明确标注是 AM312/313/314 的同动物 technical replicate；我们从样本名后缀 + 实验设计常识推断。
**敏感性**: 含 D 主分析 343 上 / 122 下，剔 D 主分析 144 上 / 10 下，一致率仅 32%。
**是否真异常**: 推断本身合理（业界 GSM 命名约定），但严格说**未经数据原作者确认**。
**建议处理**:
1. 给 GSE161878 投稿者发邮件确认 D-suffix 定义
2. 论文中 Supplementary 同时给出含 D 与剔 D 双版本，让读者自行判断
3. 若联系不到，至少在 Methods 写明: "samples with 'D' suffix were inferred to be technical replicates and excluded; sensitivity analysis with both versions provided in Supplementary"

---

#### Issue #4 — RTX 救援 28→1 衰减 🟠 中等

**现象**: GSE296065 单细胞 28 个候选通过 specificity，最终只有 **Cx3cr1 一个**通过 BH 双判 (V7 vs R7 log2FC=0.86, FDR<0.05)。
**机制解读**: RTX 阻断 TRPV1+ 神经元后，**外周 Mo Cx3cr1 表达回升**——但其余 57 个候选的 RTX 响应**未达显著**。这不一定是 RTX 救援"失败"，而是 RTX 通过单一中枢节点（Cx3cr1）发挥效应，并非全面救援。
**是否真异常**: **不是异常**，但是**很容易被过度包装**的点。
**建议处理**:
1. **不能**说"58 严选都是 RTX 靶点"
2. 应说: "Cx3cr1 是 RTX 救援的核心枢纽节点；其余候选在单细胞水平 RTX 响应较弱或未达显著（统计 power 限制 + 真实效应可能差异）"
3. 论文 figure 把 28 → 1 的漏斗画出来，让读者直观看到这是聚焦发现而非全面救援

---

#### Issue #5 — TX91 是带毒 baseline 🟡 低

**现象**: "轻症维持→重症崩塌"模型把 TX91 当 t=0，但 TX91 是 H1N1 弱毒（0.6 LD50），本身已是感染态。
**实测**: Sham mean = 10.05, TX91 = 9.88, PR8_0.6 = 9.40, PR8_10 = 8.30, PR8_100 = 7.50（Cx3cr1 例）
**含义**: 我们筛出的 564 是"**剂量响应崩塌基因**"，**不是"纯感染响应基因"**。这是 by design，但论文必须区分两种 baseline。
**建议处理**:
1. Figure legend / Methods 明确写: "Reference baseline = TX91 (sub-lethal H1N1)"
2. 不能跟 Sham vs Infection 的标准 DEG 列表混淆使用
3. 若审稿人要求 Sham baseline 版本，可同时提供 (script 03 已生成全 4 contrasts)

---

#### Issue #6 — Cx3cr1+ microglia pct=100% 是定义同义反复 🟡 低

**现象**: NTS Cx3cr1+ 微胶质 215 cells 是按 `Cx3cr1 expression > 0` 选出的，自然 pct=100%（tautological）。
**是否真异常**: 不是异常（这是定义池），但很容易被误读为"100% 验证"。
**建议处理**:
1. 论文里写: "215 cells form the operationally-defined Cx3cr1+ pool; pct=100% by construction, not validation"
2. 如要做"独立 Cx3cr1 表达验证"，应用未参与定义的额外细胞群比较

---

#### Issue #7 — Mo Cx3cr1 slope 参考点 🟡 低

**现象**: slope = -0.80 是把 TX91 当 t=0 算的；如果以 Sham 为参照，整体下降幅度数字不同。
**当前数字**: Sham 10.05 → TX91 9.88 → PR8_100 7.50（**整体跨度 -2.55**, 但单从 TX91 起算只有 -2.38）
**是否真异常**: 不是异常（slope 的数学定义清晰），但需明确告知参考点。
**建议处理**: 报告 slope 时同时给 (Sham, TX91) 双 baseline 数字，避免读者误解。

---

#### Issue #8 — 5 cells StepA 候选两两交集 = 0 🟠 中等

**现象**: 5 cells 各自的严选候选两两交集为 0（详 §9.4-X）。
**两种解读**:
- **解读 A (我们当前)**: 不同细胞看不同机制 — 生物学合理（Mo 看 Cx3cr1, Am 看 Cx3cl1, Ne 看 antimicrobial, Ly 看 lymphocyte programs）
- **解读 B (审稿人可能问)**: 5 步流程的阈值组合 (slope FDR<0.10 + Step1-5 + StepA 阈值) 太严格，对细胞特异性过敏，把"普适性保护基因"过滤掉了

**当前未做**: sensitivity 分析（如改用 cell-内 rank 而非全局阈值，看是否还会得到 0 交集）。
**建议处理**:
1. 论文 Discussion 主动两种解读并陈
2. 加做 sensitivity: 5 cells 各取 trend slope top 5%（不设 FDR 阈值）后再求交集，看是否 > 0。若 > 0，说明阈值确实过滤了普适基因；若仍 = 0，强支持解读 A。

---

#### Issue #9 — 同事 Ly 结果是噪音 ✅ 已诊断

**现象**: 同事用 GSE42639_detection.tsv（错误输入）跑出 Ly cells 是"主细胞"。
**诊断**: 见 §9.4-Y / §9.4.Y.2。同事方法 5×4=20 格里加起来共 5 个 DEG（Am=2, Mo=2, Ly=1, Ne=Nh=0）。Ly "主导"仅在 TX91_vs_Sham 这一栏（1 个 DEG, 其它 0），换 contrast 主细胞就跳。完全是 detection-p 输入下的统计噪音。
**当前状态**: 已写完整诊断报告 `reports/colleague_method_Ly_vs_Mo_review.md`。
**建议处理**: 把诊断报告 + 修复建议（intensity.tsv + neqc + limma）转给同事即可。
...
| 维度 | 评级 | 说明 |
|---|---|---|
| 假设设计 | A− | 三段拆分清晰，但原始 vagus→Mo 方向需事后修正（Issue #2） |
| 数据集对应 | A | 6 个数据集精确覆盖三段假设（详 §0.5.2） |
| 推进逻辑 | A− | Step 1→7 链路紧密（详 §0.5.3）；事后修正诚实标注 |
| 结果客观性 | B+ | 主结论稳健，但中枢端 GSE268741 单 GSM 限制因果推断（Issue #1） |
| 异常诚实度 | A | 9 项异常全部主动列出 + 给处理路径 |

**总评：A−**（机制链合理 + 主结论稳健 + 异常透明，但 Issue #1 + #2 是当前最需要补强的两点）
