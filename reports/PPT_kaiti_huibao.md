...
paginate: true
size: 16:9
header: '靶向"外周肺-中枢孤束核"神经免疫互作轴的流感后咳嗽机制研究'
footer: 'OptiSyn 项目汇报 · 2026-05-03'
style: |
  section { font-family: "PingFang SC", "STHeiti", "Microsoft YaHei", sans-serif; font-size: 22px; }
  section.lead { text-align: center; }
  h1 { color: #1a3c6e; }
  h2 { color: #2a5a9e; border-bottom: 2px solid #2a5a9e; padding-bottom: 4px; }
  h3 { color: #2a5a9e; }
  table { font-size: 18px; }
  th { background: #e8eef8; }
  blockquote { background: #fff8e6; border-left: 3px solid #f0a020; padding: 4px 10px; }
  strong { color: #1a3c6e; }
...
### 流感后慢性咳嗽 (Post-influenza chronic cough)

- 流感感染急性期过后 **>3 周持续咳嗽**，临床困境长期未解
- 单纯抗病毒 / 镇咳治疗**效果有限** → 需要新的机制解释
...
> **本研究核心**: 系统刻画 **Mo (Cx3cr1+) → vagus → NTS Tac1+/Cx3cr1+ 微胶质** 完整环路，定位中药多靶点干预节点
...
![w:900](../results/DEG_5cell_dose/analysis1_5cell_DEG_barplot.png)
...
![w:600](../results/DEG_5cell_dose/analysis2_Mo_dose_DEG_lineplot.png)
...
### 生物学定义
> "TX91 轻症维持 → PR8 剂量上升持续下降 → 100LD50 显著崩塌"
...
`Cx3cr1`, `Klf2`, `Arrb1`, `Abcd2`, `Mbp`, `Il16`, `Cd177`, `Rasgrp2`, `Rnf144a`, `Bcl11a`, **`Btla`**, **`Cd209a`**, `Rnase2b`, `Ampd3`, `Rasgrp4`, `Dock8`, `Rin3`
...
> **Cx3cr1 / Gstm1 / Dusp18** 三个核心靶点对应莲花清瘟多靶点干预（详见 Slide 18）
...
**关键修复**: AM312D/313D/314D 是同动物技术重复 → 主分析剔除 (11+10) → 144 上 / 10 下
> 含 D 版 343/122 仅 32% 一致 → 同动物伪重复污染必须剔除
...
- **vagus Cx3cl1 CPM 2.5–8.5, 21/21 样本固有表达** (IAV 感染不上调, 是固有信号源)
- **Mo Cx3cr1 重症崩塌** slope=−0.80 → 受体端故障
- 双数据集独立支持: NTS sc 模块 4 也得到 Cx3cl1→Cx3cr1 prob=0.602

> ⚠️ **诚实标注**: v1→v3 是 post-hoc refinement, 论文 Methods 必按时间顺序写
> 建议补 vagal Cx3cl1 蛋白染色 (IHC) 独立验证

---

## Slide 13 — NTS 中枢端: GSE268741 5 模块（模块 1: 分群）
...
- **关键子集**:
  - **Tac1+ Vglut2+ 兴奋性神经元: 212 cells**
  - **Cx3cr1+ 微胶质: 215 cells**
  - 其它神经元: 2,472 cells
...
> ⚠️ **单 GSM 限制**: 9,638 cells 来自 1 GSM 样本, 严格意义上属 pseudoreplication
> 论文必须明确标注 + **务必寻找第 2 份独立 NTS sc 数据**补强
...
- **RTX (TRPV1+ 神经阻断)** → 外周 Mo Cx3cr1 表达回升 → 中枢→外周反馈被阻断后保护轴恢复
- 28 → 1 漏斗 → **不能宣称"58 严选都是 RTX 靶点"**, 只 Cx3cr1 是核心枢纽
...
| 靶点 | 数据集贯穿 | 机制 |
|---|---|---|
| **Cx3cr1** ★★★ | 5 个 | RTX 救援唯一通过, slope=−0.80, 受体端枢纽 |
...
### C 档 (探索性)
- **Gstm1** — 跨毒株时序 4 双标核心, 抗氧化
- **Dusp18 / Insr / 6430548M08Rik** — 跨毒株双标
- **Gpr151** — vagal nociceptor 标志, 与"咳嗽"主线契合
...
> **莲花清瘟 = 多靶点联合介入 H1+H2+H3 三段闭环**
> 不是单一抗病毒, 而是同时缓解外周 Mo 保护轴衰退 + vagus 端致敏 + 中枢 NTS 神经-免疫反馈

---

## Slide 19 — 后续四大实验模块（湿实验设计）

| 模块 | 核心方法 | 关键评分 / 终点 | 模块创新性 |
|---|---|---|---|
| **① 体内药效学** | LD10/3 → LD50/3 + WBP 体描记 | 咳嗽次数 / 强度 / 吸气幅度 | 标准基线 |
| **② 外周机制验证** | 多色流式细胞 + scRNA-seq + 蛋白印迹 + qPCR | Cx3cr1+ Mo 数量, **Cx3cr1/Klf2/Arrb1** 表达 | 锁定 Mo 单细胞验证 |
| **③ 中枢放大验证** | 多色 IHC + Luminex + 磷酸化通路 | NTS Tac1+ ↔ Cx3cr1+ 信号通路, IL-6/TNF | NTS 区域通讯 |
| **④ 化学遗传学体内联合验证 ★** | **DREADDs + Tac1-Cre + AAV-DIO-hM4Di + i.p. CNO** | 抑制 Tac1+ 神经元 → 咳嗽是否被阻断 | **本项目最大创新点** |

### 模块 ④ 是黄金标准

- **DREADDs** (Designer Receptors Exclusively Activated by Designer Drugs)
- AAV1-DIO-hM4D(Gi)-mCherry → Tac1-Cre 小鼠 NTS
- i.p. CNO 抑制 Tac1+ 神经元活性
- 直接因果性证明: NTS Tac1+ 是流感咳嗽的中枢驱动节点
...
| # | 风险 | 严重度 | 应对 |
|---|---|:---:|---|
| 1 | **GSE268741 单 GSM** 无生物学重复 | 🔴 | 寻找第 2 份独立 NTS sc 数据 (Allen Brain / 自有协作组) |
...
3. **下一步**: DREADDs Tac1+ 抑制实验 + 莲花清瘟多靶点干预 = 黄金标准 in vivo 验证

### 已完成产出

- 6 个数据集预处理 + DEA + 多组学整合
- 14 个分析脚本 (`scripts/02–20`)
- 36 张图 + 2 份完整报告
- GitHub 项目: `OptiSyn` (commit `4431654`)

### 致谢

感谢导师指导 + 实验室同仁 + GEO 数据贡献者
...
1. 关于 NTS 单 GSM 限制，您建议优先用 Allen Brain 还是再做一份独立 sc 实验？
2. DREADDs 实验的 Tac1-Cre 小鼠系是否有现成协作单位？
3. 莲花清瘟多靶点干预的"剂量爬升"实验设计 (LD10/3 vs LD50/3) 是否合理？

**联系**: chekugpt@gmail.com
